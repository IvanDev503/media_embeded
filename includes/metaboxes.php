 ...
 */
function cmp_human_filesize($bytes) {
    if ($bytes <= 0) {
        return '0B';
    }
    $units = ['B', 'KB', 'MB', 'GB', 'TB'];
...
function cmp_admin_assets($hook) {
    ...
    wp_enqueue_style('cmp-style', CMP_PLUGIN_URL . 'assets/style.css', [], '1.0.1');
    ...
    wp_enqueue_script('cmp-admin', CMP_PLUGIN_URL . 'assets/admin.js', ['jquery', 'dropzone'], '1.0.1', true);
    wp_localize_script('cmp-admin', 'cmp_admin_ajax', [
        ...
        'remove_label' => __('Eliminar', 'carpetas-multimedia-pro'),
    ]);
}
...
function cmp_render_metabox($post) {
    ...
    $limit_str = sprintf(
        /* translators: 1: plugin limit, 2: php upload limit */
        __('Límite plugin: %1$s por archivo. Límite del servidor/PHP: %2$s', 'carpetas-multimedia-pro'),
        '1GB',
        cmp_human_filesize($php_limit)
    );
    wp_nonce_field('cmp_metabox_nonce', 'cmp_metabox_nonce_field');
    ?>
    <div class="cmp-metabox-wrapper">
        <div class="notice notice-info inline"><p><?php echo esc_html(__('Los archivos se agregan directo a la biblioteca de medios y quedan disponibles al instante. Solo podrás eliminarlos desde aquí si necesitas retirarlos de la carpeta.', 'carpetas-multimedia-pro')); ?></p></div>
        <div class="cmp-dropzone" id="cmp-dropzone-images" data-type="image" data-post="<?php echo esc_attr($post->ID); ?>">
            ...
            <p><?php esc_html_e('Formatos permitidos: JPG, PNG, GIF, WebP. Sin moderación: se publican directo en la biblioteca.', 'carpetas-multimedia-pro'); ?></p>
            <p class="description"><?php echo esc_html($limit_str); ?></p>
        ...
        <div class="cmp-dropzone" id="cmp-dropzone-documents" data-type="document" data-post="<?php echo esc_attr($post->ID); ?>">
            ...
            <p><?php esc_html_e('Formatos permitidos: PDF, DOCX, XLSX, ZIP, PPTX. Se publican inmediato en la biblioteca multimedia.', 'carpetas-multimedia-pro'); ?></p>
            <p class="description"><?php echo esc_html($limit_str); ?></p>
            <div class="cmp-progress" aria-hidden="true">
                <div class="cmp-progress-bar"></div>
                <span class="cmp-progress-text">0%</span>
